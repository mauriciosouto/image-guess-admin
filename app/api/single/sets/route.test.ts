...
describe("GET /api/single/sets", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });

  it("returns { sets } from getPlayableFabSetLabels", async () => {
    ...
    expect(body.sets).toEqual(["History Pack 1", "Welcome to Rathe"]);
    expect(getPlayableFabSetLabels).toHaveBeenCalledTimes(1);
  });

  it("returns 500 when query fails", async () => {
    getPlayableFabSetLabels.mockRejectedValueOnce(new Error("db"));
